...
export async function generatePDF(quoteData: QuoteData, formData: any) {
  ...
  const primaryColor: [number, number, number] = [13, 148, 136]; // teal-600 en RGB
  const textColor: [number, number, number] = [15, 23, 42]; // slate-900 en RGB
  const lightTextColor: [number, number, number] = [100, 116, 139]; // slate-500 en RGB
  ...
  doc.text(cleanString(formatCurrency(quoteData.subtotal)), 195, currentY, {
    align: "right",
  });
  ...
  if (quoteData.urgentFee) {
    ...
    doc.text(cleanString(formatCurrency(quoteData.urgentFee)), 195, currentY, {
      align: "right",
    });
  }
  ...
  doc.text(cleanString(formatCurrency(quoteData.tax)), 195, currentY, {
    align: "right",
  });
  ...
  doc.text(cleanString(formatCurrency(quoteData.total)), 195, currentY, {
    align: "right",
  });
  ...
  // Save the PDF
  doc.save(
    `Devis_${formData.firstName}_${formData.lastName}_${
